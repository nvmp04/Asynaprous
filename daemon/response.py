...
class Response():   
    ...
    def build_response(self, request, envelop_content=None):
        ...
        # ── Basic Auth check ──────────────────────────────────────────
        if not (hasattr(request, 'hook') and request.hook):
        # Bỏ qua Basic Auth cho file tĩnh HTML/CSS/JS
            path = request.path
            mime_type = self.get_mime_type(path)
            if not (path.endswith('.html') or mime_type in ('text/html', 'text/css', 'application/javascript')):
                VALID_USERS = {
                    "admin": "password",
                    "user1": "password",
                }
                ...
                if VALID_USERS.get(username) != password:
                    return self.build_unauthorized()
                print("[Response] Auth OK cho user: {}".format(username))
        #───────────────────────────────────────────────
        path = request.path
        
        if hasattr(self, 'routes') and path in self.routes:
            print(f"[Response] Found API handler for {path}")
        ...
        # Build header then assemble full response
        self._header = self.build_response_header(request)

        return self._header + self._content
    
    def build_unauthorized(self, realm="Protected"):
        """
        Constructs a 401 Unauthorized HTTP response yêu cầu Basic Auth.
        """
